...
const filteredScreenshotsForSuites = (files, suites, screenshotsFolder) => {
  return files
    .filter((s) => suites.some((suite) => s.includes(fileNameForString(suite))))
    .filter((s) => {
      const suitesInPath = s
        .slice(screenshotsFolder.length + 1)
        .split('/')
        .slice(1)
        .join('/')
        .split(' -- ')
...
const getPassedScreenshots = (screenshotsFolder, testTitle, suites) => {
  const patternBase = screenshotGlobPattern(screenshotsFolder, testTitle);
...
const getFailedScreenshot = (screenshotsFolder, testTitle, testSuites) => {
