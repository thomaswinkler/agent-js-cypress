...
const screenshotBasePattern = (testTitle, suiteTitle) => {
  const normalizedTestTitle = fileNameForString(testTitle || '');
  const normalizedSuiteTitle = fileNameForString(suiteTitle || '');
  const patternBase = `**/*${normalizedSuiteTitle.length ? normalizedSuiteTitle + '*' : ''}`;
  return `${patternBase}${normalizedTestTitle}`;
};
...
const getPassedScreenshots = (testTitle, suiteTitle) => {
  const patternBase = screenshotBasePattern(testTitle, suiteTitle);
  const patternFirstScreenshot = `${patternBase}.png`;
  const patternNumeratedScreenshots = `${patternBase} (*([0-9])).png`;
  const patternAfterEachScreenshots = `${patternBase} -- after each hook.png`;
  const firstScreenshot = glob.sync(patternFirstScreenshot) || [];
  const numeratedScreenshots = glob.sync(patternNumeratedScreenshots) || [];
  const afterEachScreenshots = glob.sync(patternAfterEachScreenshots) || [];
  const files = firstScreenshot.concat(numeratedScreenshots, afterEachScreenshots);
...
const getFailedScreenshot = (testTitle, suiteTitle) => {
  const patternBase = screenshotBasePattern(testTitle, suiteTitle);
  const pattern = `${patternBase} (failed).png`;
...
const getTestInfo = (test, testFileName, status, err) => {
  ...
  return {
    ...
    testFileName,
    tags: attributesFromTags(testTags),
  };
};
