...
const screenshotGlobPattern = (testTitle) => {
  const normalizedTestTitle = fileNameForString(testTitle || '');
  return `**/*${normalizedTestTitle}*.png`;
};
...
const getPassedScreenshots = (testTitle, suites) => {
  const patternBase = screenshotGlobPattern(testTitle);
  const allTestScreenshots = glob.sync(patternBase) || [];
  let filteredScreenshots = allTestScreenshots.filter((s) => !s.match(/\(failed\)\.png$/));

  if (suites) {
    filteredScreenshots = filteredScreenshots.filter((s) =>
      suites.some((suite) => s.includes(fileNameForString(suite))),
    );
  }
...
const getFailedScreenshot = (testTitle, suites) => {
  const patternBase = screenshotGlobPattern(testTitle);
  const allTestScreenshots = glob.sync(patternBase) || [];
  let filteredScreenshots = allTestScreenshots.filter((s) => s.match(/\(failed\)\.png$/));
