...
// https://docs.cypress.io/api/commands/screenshot#Naming-conventions
const getCustomScreenshot = (absolutePath) => {
...
const getCustomScreenshots = (screenshotsFolder, customScreenshotsFileNames, specFilePath) => {
  ...
  return [...new Set(customScreenshotsFileNames)].reduce((screenshots, screenshotFilename) => {
    let screenshotFiles = glob.sync(`${base}/${specFileName}/${screenshotFilename}*.png`) || [];
    if (screenshotFiles.length > 1) {
      screenshotFiles = screenshotFiles.filter((f) => f.includes('attempt'));
    }
    const file = screenshotFiles.pop();
    if (file) {
