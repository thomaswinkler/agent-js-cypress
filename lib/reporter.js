...
class Reporter {
  ...
  sendLog(tempId, { level, message = '', file }) {
    ...
    if (logMessage.trim().startsWith('cy:screenshot')) {
      const [log, ...rest] = logMessage.split(/\r?\n/);
      if (rest && rest.length) {
        try {
          const options = JSON.parse(rest.join('\n'));
  ...
  sendScreenshot({ fileName }) {
    ...
    if (fileName && tempItemId) {
      const file = getCustomScreenshot(fileName);
      ...
      promiseErrorHandler(sendScreenshotsPromise, 'Fail to save screenshot with log item.');
    }
  }
