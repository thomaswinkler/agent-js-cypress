...
class Reporter {
  ...
  sendLogOnFinishItem(test, tempTestId) {
    ...
    const suiteTitle = this.getCurrentSuiteInfo() && this.getCurrentSuiteInfo().title;
    const passedScreenshots = getPassedScreenshots(test.title, suiteTitle);
  ...
  sendLog(tempId, { level, message = '', file }) {
    ...
    if (message.match(/cy:command[\s-]+screenshot/)) {
      try {
        const splitMessage = message.split(/\s/);
        const index = splitMessage.indexOf('screenshot');
        if (splitMessage.length > index) {
          const screenshotName = fileNameForString(splitMessage.slice(index + 1).join(' '));
          const specFileName = path.parse(this.currentTestTempInfo.test.testFileName).base;
          const screenshot = glob.sync(`**/${specFileName}/${screenshotName}.png`) || [];
          if (screenshot.length > 0) {
            screenshotFile = {
              name: screenshotName,
              type: 'image/png',
              content: base64Encode(screenshot[0]),
            };
          }
        }
      } catch (error) {
