...
class Reporter {
  ...
  sendVideoOnFinishSuite(suite) {
    if (suite && this.suitesStackTempInfo.length && suite.id === this.suitesStackTempInfo[0].id) {
      ...
      if (this.suitesStackTempInfo[0].status !== testItemStatuses.FAILED && !videoUploadOnPasses) {
        console.log(`Video not uploaded (videoUploadOnPasses=false)`);
        return;
      }

      const testFileName = this.suitesStackTempInfo[0].testFileName;
      if (testFileName != null) {
        const pathComponents = testFileName.split('/');
        const specFileName = pathComponents.pop();

        const videoFileDetails = getVideoFile(specFileName);
        if (videoFileDetails) {
          const suiteId = this.testItemIds.get(suite.id);
  ...
  sendLogOnFinishItem(test, tempTestId) {
    ...
    if (test.status === FAILED) {
      const sendFailedLogPromise = this.client.sendLog(
        ...
        getFailedScreenshot(
          this.config.reporterOptions.screenshotsFolder,
          test,
          this.suitesStackTempInfo.map((suite) => suite.title),
        ),
      ).promise;
      promiseErrorHandler(sendFailedLogPromise, 'Fail to save error log');
    }

    const passedScreenshots =
      getPassedScreenshots(
        this.config.reporterOptions.screenshotsFolder,
        test,
        this.suitesStackTempInfo.map((suite) => suite.title),
    ...
    // console.log(JSON.stringify(passedScreenshots.concat(customScreenshots), undefined, 2));
    passedScreenshots.concat(customScreenshots).forEach((file) => {
      ...
      promiseErrorHandler(sendScreenshotsPromise, 'Fail to save passed log');
    });
  }
  ...
  testPending(test) {
    // check if test has started. if not, save test.id to finish test when
    // started in testStart(). If started, just finish the test.
    // in all tests, TEST_PENDING event has ALWAYS come up before TEST_START
  ...
  sendLog(tempId, { level, message = '', file }) {
    let logTimestamp = new Date().valueOf();
    let logMessage = message;

    // if message contains a timestamp, use this one instead of current date
    ...
    if (rest && rest.length > 0) {
      ...
      logMessage = rest.join('@');
    }

    // attach screenshot into the log, so it appears with name at the expected position
    let screenshotFile;
    if (logMessage.trim().startsWith('cy:screenshot')) {
      const [log, ...optionLines] = logMessage.split(/\r?\n/);
      if (optionLines && optionLines.length) {
        try {
          const options = JSON.parse(optionLines.join('\n'));
          screenshotFile = getCustomScreenshot(options.path);
        } catch (error) {
          console.log(`Error attaching screenshot to log message: ${error}`);
        }
      } else {
        try {
          const fileName = log.slice('cy:screenshot'.length).trim();
          const files = getCustomScreenshots(
            this.config.reporterOptions.screenshotsFolder,
            [fileName],
            this.currentTestTempInfo.test.testFileName,
          );
    ...
    console.log(`====> ${logMessage} -> ${screenshotFile && screenshotFile.name}`);
    // if (screenshotFile && screenshotFile.name) {
    //   logMessage += ` (${screenshotFile.name})`;
    // }
