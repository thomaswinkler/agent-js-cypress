...
const { prepareReporterOptions, validateReporterOptions } = require('./../utils');
...
const registerReportPortalPlugin = (on, config, callbacks) => {
  ...
  on('task', {
    ...
    rp_setLaunchDescription(description) {
      emit(IPC_EVENTS.SET_LAUNCH_DESCRIPTION, description);
      return null;
    },
  });
  ...
  on('after:screenshot', (details) => {
    ...
    emit(IPC_EVENTS.SCREENSHOT, details);
    return null;
  });
};
